...
setup-desktop() {
	...
	if ask "Install Hyprland?"; then
		sudo yay -S --needed --noconfirm hyprland
...
network-packages() {
	...
	if ask "Install ldns and ufw?"; then
		sudo pacman -S --needed --noconfirm ldns ufw
	fi
...
misc-packages() {
	...
	if ask "Install PDF reader (zathura)?"; then
		...
		sudo pacman -S --needed zathura-pdf-mupdf
	fi
...
fonts() {
	...
	if ask "Install ttf-font-awesome and ttf-fira-code-font?"; then
		...
		yay -S --needed --noconfirm nerd-fonts-fira-code
...
game() {
	...
	if ask "Install LÖVE (Love2D)?"; then
		sudo pacman -S --needed --noconfirm love
	fi
}

latex() {
	if ask "Install latexmk?"; then
		sudo pacman -S --needed --noconfirm latexmk
	fi
...
developer-packages() {
	if ask "Install C development tools?"; then
		sudo pacman -S --needed --noconfirm gcc gdb make cmake
	fi
	...
	if ask "Install Python and related tools?"; then
		sudo pacman -S --needed --noconfirm python python-pip python-virtualenv pipx
	...
	if ask "Install LuaRocks (Lua package manager)?"; then
		sudo pacman -S --needed --noconfirm luarocks lua
	fi
...
if ask "Install basic packages (git, curl, etc)?"; then
	sudo pacman -S --needed --noconfirm git github-cli curl tldr fzf wl-clipboard unzip base-devel ripgrep
